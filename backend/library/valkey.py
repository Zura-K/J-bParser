...
class _Commands:
    ...
    def _call(self, command: str, *args, transform=None, **kwargs):
    ...
    def get(self, key: str) -> str | None:
        return self._call("get", self._prefixed(key), transform=_text_or_none)

    def set(self, key: str, value, ttl_seconds: int | None = None):
        return self._call("set", self._prefixed(key), value, ex=ttl_seconds)
    ...
    def get_bytes(self, key: str) -> bytes | None:
        return self._call("get", self._prefixed(key))

    def set_bytes(self, key: str, value: bytes, ttl_seconds: int | None = None):
        return self._call("set", self._prefixed(key), value, ex=ttl_seconds)

    def delete(self, *keys: str):
        return self._call("delete", *(self._prefixed(key) for key in keys))

    def exists(self, key: str) -> bool:
        return self._call("exists", self._prefixed(key), transform=bool)

    def increment(self, key: str, amount: int = 1) -> int:
        return self._call("incrby", self._prefixed(key), amount)

    def expire(self, key: str, seconds: int):
        return self._call("expire", self._prefixed(key), seconds)

    def persist(self, key: str):
        return self._call("persist", self._prefixed(key))
    ...
    def ttl(self, key: str) -> int:
        return self._call("ttl", self._prefixed(key))

    def rename_key(self, source: str, target: str):
        return self._call("rename", self._prefixed(source), self._prefixed(target))
    ...
    def get_collection(self, key: str):
        return self._call("get", self._prefixed(key), transform=_collection_or_none)

    def set_hash(self, key: str, fields: dict, ttl_seconds: int | None = None):
        self._call("hset", self._prefixed(key), mapping=_encode_hash_fields(fields))
        return self.apply_ttl(key, ttl_seconds)

    def get_hash(self, key: str) -> dict | None:
        return self._call("hgetall", self._prefixed(key), transform=_hash_or_none)

    def delete_hash_fields(self, key: str, *names: str):
        return self._call("hdel", self._prefixed(key), *names)

    def add_to_set(self, key: str, *members: str, ttl_seconds: int | None = None):
        self._call("sadd", self._prefixed(key), *members)
        return self.apply_ttl(key, ttl_seconds)

    def remove_from_set(self, key: str, *members: str):
        return self._call("srem", self._prefixed(key), *members)

    def set_members(self, key: str) -> set[str]:
        return self._call("smembers", self._prefixed(key), transform=_text_set)

    def in_set(self, key: str, member: str) -> bool:
        return self._call("sismember", self._prefixed(key), member, transform=bool)

    def add_scored(self, key: str, mapping: dict):
        return self._call("zadd", self._prefixed(key), mapping)
    ...
    def trim_by_score(self, key: str, low, high):
        return self._call("zremrangebyscore", self._prefixed(key), low, high)

    def append_to_list(self, key: str, *values):
        return self._call("rpush", self._prefixed(key), *values)

    def remove_from_list(self, key: str, value, count: int = 1):
        return self._call("lrem", self._prefixed(key), count, value)
    ...
    def merge_sets(self, destination: str, *sources: str):
        ...
        return self._call("sunionstore", self._prefixed(destination), prefixed)


class XValkey(_Commands):
    ...
    def remember(self, key: str, ttl_seconds: int | None, producer: Callable):
        ...
        if raw is not None:
            ...
            value = producer()
            self.set_collection(key, value, ttl_seconds)
            if not value:
                value = None
    ...
    def scan_keys(self, pattern: str, count: int = 200) -> list[str]:
        ...
        cursor = 0
        while True:
            cursor, batch = self._call(
                "scan", cursor, match=self._prefixed(pattern), count=count
            )
            found.extend(self._unprefixed(_text(key)) for key in batch)
            if cursor == 0:
                return found
...
class XValkeyPipeline(_Commands):
    def __init__(self, owner: XValkey):
        self._owner = owner
        self._prefix = owner._prefix
